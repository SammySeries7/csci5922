...
class myneuronnet():
    def __init__(self):
        ...
        self.OutputSize = 10
        self.HiddenUnits = 5  ## one layer with h units
        self.n = 2000  ## number of training examples, n
    ...
    def FeedForward(self,x):
        ...
        y_hat=self.sigmoids(self.z2,dS=False)
    ...
    def BackProp(self,x,y,y_hat):
            self.eta=0.5
            error=y_hat-y
            y_hat_d_error=self.sigmoids(y_hat,dS=True)*error
            #dl/dh=w2*(y^)(1-y^)*(y^-y)
            dl_dh=np.dot(y_hat_d_error,self.w2.T)
            #dl/dh=(h1)(1-h1)*w2*(y^)(1-y^)*(y^-y)
            dl_dz1=self.sigmoids(self.a_z1,dS=True)*dl_dh
            #dl/dw2=h.T*(y^)(1-y^)*(y^-y)
            dl_dw2=np.dot(self.a_z1.T,y_hat_d_error)
            ...
            print('dl/db',np.mean(dl_dz1,axis=0))
            self.bs=self.bs-self.eta*np.mean(dl_dz1,axis=0)
            print('bs_new: ',self.bs)
            print('dl/dc',np.mean(y_hat_d_error,axis=0))
            self.c=self.c-self.eta*np.mean(y_hat_d_error,axis=0)
            print('c_new: ',self.c)
            print('y_hat: ',y_hat)
    ...
    def PredictNetwork(self, x, y):
        ...
        print('Confusion matrix: ',confusion_matrix(y_test_n,output_n))
